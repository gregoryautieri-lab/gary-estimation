/**
 * Logos GARY en SVG inline pour le PDF
 * Extraits exactement du fichier source original
 */

// Logo GARY blanc (pour headers sur fond sombre)
...
// Logo GARY rouge (pour footers sur fond clair ou sur fond sombre)
...
/**
 * Retourne le logo avec une taille spécifique appliquée
 * @param variant - 'white' ou 'red'
 * @param height - Hauteur en pixels
 * @returns String HTML du logo SVG avec style inline
 */
export function getLogo(variant: 'white' | 'red', height: number = 28): string {
  const baseSvg = variant === 'white' ? logoWhite : logoRed;
  return baseSvg.replace('viewBox', `style="height:${height}px;width:auto;" viewBox`);
}
