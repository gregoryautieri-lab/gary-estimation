...
function generateTrajectoiresPage(estimation: EstimationData, pageNum: number = 4, totalPages: number = 9): string {
  ...
  const getTimelinePhaseConfig = (type: string) => {
    switch (type) {
      case 'offmarket':
        return [
  ...
  trajectoires.forEach((traj) => {
    ...
    if (luxMode) {
      ...
      html += '<div style="padding:8px 10px;background:white;text-align:center;margin-top:auto;">';
      ...
      // Objectif de valeur
      html += '<div style="padding:12px;background:white;text-align:center;margin-top:auto;">';
